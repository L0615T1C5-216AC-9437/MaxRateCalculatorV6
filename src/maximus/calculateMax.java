...
public class calculateMax extends mrc.calculation {
            ...
            for (int y = yb; y <= yt; y++) {
                ...
                if (t.block() instanceof GenericCrafter gc) {
                    if (gc.outputItem != null) {
                        itemProduction(gc.outputItem, gc.craftTime);
                    }
                ...
                if (t.block() instanceof OverdriveProjector op) {
                    if (op.consumes.has(ConsumeType.item) && op.consumes.get(ConsumeType.item) instanceof ConsumeItems ci) {
                        itemConsumption(ci.items, op.useTime, ci.isOptional());
                    }
                    ...
                    if (mp.consumes.has(ConsumeType.item) && mp.consumes.get(ConsumeType.item) instanceof ConsumeItems ci) {
                        itemConsumption(ci.items, mp.useTime, ci.isOptional());
                    }
                }
                repeat.add(t.build);
            }
    ...
    private void itemProduction(ItemStack stack, float interval) {
        itemPC.putIfAbsent(stack.item, new ArrayList<>());
        itemPC.get(stack.item).add(new pcEntry(stack.amount / (interval / 60f), 0, false));
    }
